...
const ComponentPreview: React.SFC<Props> = props => {
  ...
  return (<div className={clsx(classes.root)}>
    {images && images.length ? (
      <div className={clsx(classes.imageContainer)}>{images.map((imageObject: any) => (<img
        className={classes.image}
        src={`//${imageObject.defaultHost}/i/${imageObject.endpoint}/${imageObject.name}`}
      />))}
      </div>) : null}
    {text ? (<Typography className={clsx(classes.text)} variant="body1">{text}</Typography>) : null}
  </div>);
};
